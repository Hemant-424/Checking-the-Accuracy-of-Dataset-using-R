...
a<-cor(dataset[,1:13])
plot(corrplot(a,method = 'ellipse',type = 'full'))
...
par(mfrow=c(2,7))
for(i in 1:13) {
	hist(dataset[,i], main=names(dataset)[i],col=terrain.colors(7))
}

# density plot for each attribute
par(mfrow=c(2,7))
for(i in 1:13) {
	plot(density(dataset[,i]), main=names(dataset)[i],col=brewer.pal(8,"Set3"))
}

# boxplots for each attribute
par(mfrow=c(2,7))
for(i in 1:13) {
	boxplot(dataset[,i], main=names(dataset)[i],col = terrain.colors(7))
}
...
corrplot(correlations, method="ellipse",type = 'full')
...
summary(results)
dotplot(results)
#barchart(results,groups = TRUE,origin = 0)
